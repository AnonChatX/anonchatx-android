...
import org.anonchatx.android.DestroyableContext;
...
import org.anonchatx.android.widget.TapSafeFrameLayout;
import org.anonchatx.android.widget.TapSafeFrameLayout.OnTapFilteredListener;
import org.anonchatsecure.anonchat.api.android.ScreenFilterMonitor;
import org.anonchatsecure.anonchat.api.android.ScreenFilterMonitor.AppDetails;
...
import static org.anonchatx.android.TestingConstants.PREVENT_SCREENSHOTS;
import static org.anonchatx.android.util.UiUtils.hideSoftKeyboard;
import static org.anonchatx.android.util.UiUtils.showFragment;
...
public abstract class BaseActivity extends AppCompatActivity
	...
	private static final Set<String> NO_ACTION_BAR_ACTIVITIES = new HashSet<>(Arrays.asList(
			"StartupActivity",
			"SetupActivity",
	...
	public void onCreate(@Nullable Bundle state) {
		...
		AndroidComponent applicationComponent =
				((AnonChatApplication) getApplication()).getApplicationComponent();
	...
	private boolean showScreenFilterWarning() {
		if (((AnonChatApplication) getApplication()).isInstrumentationTest()) {
			return false;
		}
	...
	private void applyCustomTheme() {
		...
		boolean noActionBar = NO_ACTION_BAR_ACTIVITIES.contains(getClass().getSimpleName());

		if (theme == null) {
			// Fallback theme to avoid ActionBar conflict
			setTheme(noActionBar ? R.style.AnonTheme_Base_NoActionBar : R.style.AnonTheme_Base);
			return;
		}

		if (theme.equals(getString(R.string.pref_theme_dark_value))) {
			setTheme(noActionBar ? R.style.AnonTheme_Base_NoActionBar : R.style.AnonTheme_Base);
		} else if (theme.equals(getString(R.string.pref_theme_light_value))) {
			setTheme(noActionBar ? R.style.AnonTheme_Base_NoActionBar : R.style.AnonTheme_Base);
		} else if (theme.equals(getString(R.string.pref_theme_anon_green_dark_value))) {
			setTheme(noActionBar ? R.style.AnonGreenTheme_NoActionBar : R.style.AnonGreenTheme);
		} else if (theme.equals(getString(R.string.pref_theme_anon_green_light_value))) {
			setTheme(noActionBar ? R.style.AnonGreenTheme_NoActionBar : R.style.AnonGreenTheme);
		} else if (theme.equals(getString(R.string.pref_theme_discrete_blue_dark_value))) {

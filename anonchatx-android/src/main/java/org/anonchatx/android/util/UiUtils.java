...
import org.anonchatx.android.reporting.FeedbackActivity;
import org.anonchatx.android.view.ArticleMovementMethod;
import org.anonchatsecure.anonchat.api.android.MemoryStats;
...
import static org.anonchatx.android.TestingConstants.EXPIRY_DATE;
import static org.anonchatx.android.reporting.CrashReportActivity.EXTRA_APP_LOGCAT;
import static org.anonchatx.android.reporting.CrashReportActivity.EXTRA_APP_START_TIME;
import static org.anonchatx.android.reporting.CrashReportActivity.EXTRA_INITIAL_COMMENT;
import static org.anonchatx.android.reporting.CrashReportActivity.EXTRA_MEMORY_STATS;
import static org.anonchatx.android.reporting.CrashReportActivity.EXTRA_THROWABLE;
...
public class UiUtils {
	...
	public static SpannableStringBuilder getTeaser(Context ctx, Spanned text) {
		...
		ForegroundColorSpan fg = new ForegroundColorSpan(
				ContextCompat.getColor(ctx, R.color.anon_text_link));
	...
	public static void setTheme(Context ctx, String theme) {
		...
		} else if (theme.equals(ctx.getString(R.string.pref_theme_anon_green_light_value))) {
			setDefaultNightMode(MODE_NIGHT_NO);
			ctx.setTheme(R.style.AnonGreenTheme);
		} else if (theme.equals(ctx.getString(R.string.pref_theme_discrete_blue_dark_value))) {
			setDefaultNightMode(MODE_NIGHT_YES);
	...
	public static void launchActivityToOpenFile(
			Context context,
		...
		try {
			// Prefer docLauncher (supports multiple types)
		...
		try {
			// Fallback to first MIME type for contentLauncher
			if (mimeTypes.length > 0) {
				contentLauncher.launch(mimeTypes[0]);
				return;
			}
		} catch (ActivityNotFoundException e) {
			logException(LOG, WARNING, e);
		...
		Toast.makeText(context, R.string.error_start_activity, LENGTH_LONG).show();
	}

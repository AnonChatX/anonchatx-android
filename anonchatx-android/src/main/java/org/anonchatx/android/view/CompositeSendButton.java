...
public class CompositeSendButton extends FrameLayout {
	...
	private final AppCompatImageButton sendButton, imageButton, recordButton;
	...
	public void showSendButtonInstead(boolean showSend, boolean sendEnabled) {
		if (showSend) {
			// Show send button
			...
			recordButton.animate().alpha(0f).withEndAction(() ->
			...
			recordButton.clearAnimation();
			recordButton.animate().alpha(1f).start();
	...
	public void setMicColor(boolean distorted) {
		int color = getResources().getColor(
				distorted ? R.color.anon_green_600 : R.color.anon_orange_600,
				getContext().getTheme()
		);

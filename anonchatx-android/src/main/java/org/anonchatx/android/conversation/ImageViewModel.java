...
public class ImageViewModel extends DbViewModel implements EventListener {
	...
	public void getAttachmentBitmap(AttachmentItem attachment, Consumer<Bitmap> callback) {
		ioExecutor.execute(() -> {
			try {
				Attachment data = attachmentReader.getAttachment(attachment.getHeader());
				InputStream inputStream = data.getStream();
				Bitmap bitmap = BitmapFactory.decodeStream(inputStream);
				tryToClose(inputStream, LOG, WARNING);
				androidExecutor.runOnUiThread(() -> callback.accept(bitmap));
			} catch (Exception e) {
				Log.e("ImageViewModel", "Failed to load bitmap", e);
				androidExecutor.runOnUiThread(() -> callback.accept(null));
			}
		});
	}
